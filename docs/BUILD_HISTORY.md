...
| 26 | 2026-07-24 | **TD-01**: Alembic = single schema source of truth. Removed `create_all` + `_apply_postgres_guards` from startup; `ensure_schema()` (apply in local, verify elsewhere); Dockerfile ships migrations; SQLite-vs-Postgres test split | **134** SQLite / **142** with Postgres (+ gated) | 13 migration tests on real Postgres (fresh base→head, exclusion constraint, triggers, btree_gist, FKs, idempotency uniques, ORM parity, verify-mode raises); live app boots via migrations; money-flow smoke recon=0 | _this cycle_ | — | — |
