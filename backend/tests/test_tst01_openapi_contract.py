...
def test_every_served_route_is_in_the_contract():
    ...
    served = {
        r.path
        for r in app.routes
        if getattr(r, "include_in_schema", False) and r.path not in ("/openapi.json",)
    }
...
def test_core_endpoints_are_present():
    spec = json.loads(SPEC_PATH.read_text(encoding="utf-8"))
    for path in (
        "/v1/auth/register", "/v1/auth/login", "/v1/bookings",
        "/v1/bookings/{booking_id}/cancel", "/v1/listings",
        "/v1/payments/webhook/stripe", "/v1/hosts/{host_id}/payouts/run",
        "/v1/admin/ledger/reconciliation",
    ):
